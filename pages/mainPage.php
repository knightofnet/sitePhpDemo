            ...
            <div class="card">
                ...
                <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#indexExplications">
                    <div class="card-body">
                        ...
                        <p>Vous venez de la page de connexion <code>index.php</code>. Votre connexion a été effectuée avec succès, et donc il y a eu une redirection en utilisant la fonction PHP <code>header()</code>.</p>
                ...
                <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#indexExplications">
                    <div class="card-body">
                        ...
                        <ul>
                            ...
                            <li>Sécurité : le point précédent implique donc que les cookies ne sont pas un moyen sécurisé de sauvegarder des informations.</li>
                            ...
                            <li>Les sessions utilisent un cookie pour fonctionner : en ouvrant les outils de développement web (touche <kbd>F12</kbd>), puis en vous rendant dans l'onglet Stockage et enfin dans les cookies de <code>http://localhost</code>, vous pourrez visualiser les cookies enregistrés pour ce domaine. Un cookie est spécial : PHPSESSID. Il s'agit d'un cookie enregistré par PHP qui permet de garder l'identifiant de la session qui est sauvegardé sur le serveur. Cette valeur (<code><?= session_id() ?></code>) permet donc à PHP de savoir à qui sont les données stockées en mémoire, côté serveur, et lui réattribuer dans la variable <code>$_SESSION</code>.</li>
                        </ul>

                        <p><span class="font-weight-bold">Utilisation des sessions : </span> en inspectant le code source de ce site, avec Visual Studio Code par exemple, vous avez pu apercevoir comment fonctionnaient les sessions en PHP. Quelques rappels :
                        </p>
                        <ul>
                            <li>Déclarer l'utilisation des sessions le plus tôt possible : Pour utiliser les sessions PHP, il faut que l'instruction <code>session_start()</code> soit écrite le plus tôt possible dans votre fichier PHP. Pour ce site de démonstration, elle est écrite dans le fichier <code>initCore.php</code>, fichier qui est inclus dans toutes les pages du site. Aucun affichage/envoi de contenu ne doit avoir eu lieu avant son utilisation (donc pas de <code>echo</code>, pas de <code>print()</code> et pas de <code>var_dump()</code>). Si du contenu était envoyé avant, il y aurait une erreur indiquant <code>headers already sent</code> : si tel est le cas, il faudra vérifier votre code.</li>
