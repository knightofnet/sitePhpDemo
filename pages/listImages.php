...
$messagePourHtml = "";
// Si une erreur est passée par l'URL, avec le paramètre 'error', on va l'afficher.
if (isset($_GET['error'])) {
	$messagePourHtml = htmlspecialchars($_GET['error']);
}
	...
	<div class="col-sm">
		...
		<form class="form-signin col-sm-12" method="post" action="<?= URL_SITE ?>\php\traitements\envoiImage.php" enctype="multipart/form-data">
			...
			<div class="form-group">
				...
				<small class="form-text text-muted">Le nom de l'image, tel qu'il apparaîtra dans le site. Il peut être différent du nom du fichier de l'image.</small>
			</div>

			<div class="form-group">
				<label for="nomImage">MAX_FILE_SIZE (normalement ce champ doit être caché <code>&lt;input type="hidden" ...&gt;</code>)</label>
