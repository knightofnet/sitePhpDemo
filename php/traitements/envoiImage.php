...
// Si aucune donnée n'est soumise par un formulaire, ou si l'utilisateur n'est pas connecté, ou si la session est inexistante,
...
// Les informations du fichier sont disponibles dans la variable globale $_FILES.
...
if ($resultat) {
    ...
    if ($idImage == 0) {
        $erreur = "Erreur lors de l'ajout de l'image en BDD.";
        redirectWithError($erreur);
    }
    header("Location: " . URL_SITE . "/pages/listImages.php?msg=Ok");
    exit();
} else {
    $erreur = "Erreur lors du transfert vers le serveur.";
    redirectWithError($erreur);
}
